...
- :smile: 关于文章书写的一些规范
  - 【人称】文章是以第一人称描述，还是以第三人称描述
    - 第一人称就是we开头，we propose, we derive, we come up with
    - 第三人称就是避免出现we，而使用 
      - This paper devote to ..
      - The Lemma tells that ...
      - Denote the space as ...
      - ... is derived for 
  - 【从句】书写文章的时候尽量避免使用从句，避免which,  where
  - 【记号】最好将所有的notation放在同一个地方，而不是随用随写
    - 如果涉及到区域的符号或者有限元方法空间相关的，需要在给出有限元相关的记号后才能给出对应的符号说明，可以除了在文章开始的部分给出以外，在有限元离散后给出部分记号。
    - 如果有些记号全文只出现一次，就可以在用到的时候给出记号的定义。
  - 【引用】所有涉及推导的地方必须给出方程，定理，或者公式的引用！以避免出现混淆！
  - 【标点】所有方程中的标点符号也是需要考虑的，方程中使用逗号，如果方程后跟的是介词，方程的结束可以不加符号，如果方程是句子的结束，需要加句号。
  - 【图片】图片应该保存成fig格式，再另存为 eps, jpg 格式在latex中使用；
- 误差估计中的无穷范数的估计应该要用到归纳法的假设；
