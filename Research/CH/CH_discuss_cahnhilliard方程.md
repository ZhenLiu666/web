...
#### 231102 非线性问题解的存在唯一性1

- 讲解文章《On the Cahn-Hilliard Equation》一个半小时，一维非线性解的全局存在性
  - 解的局部存在唯一性使用Picard迭代定理得到，只需要得到解的全局有界性估计，就可以得到解的全局存在性，比如全局的$L^{\infin}$范数能被一个与时间$T$无关的常数控制即可；
  - :pushpin: 关于范数的估计，不等式的一些应用，见此文章；
  - 关于sobolev嵌入定理的一些结论，比较sobolev范数，2D，3D的区别；
- 新的证明思路：
  - 证明右端项是Lipschiz连续的；
  - 证明$u$是一致有界的，这是因为$|\phi(u(x)) - \phi(u(y))| \leq \phi'(u)|x-y|$，因此需要$\phi'(u)\leq L$，由于此时的$\phi$的表达式是$u$的函数，因此需要估计$u$的有界性；
- 数值解应该考虑间断的例子是否算的好？ 水珠的例子初值其实是光滑的；





#### 231109 非线性问题解的存在唯一性2

- 马睿师姐讲解**非线性问题混合变分问题等价性**
  - 基本思路：
    - （1）原始变分问题的解是存在唯一的；
    - （2）新的混合变分解是存在唯一的；
    - （3）原始变分问题的解满足新的混合变分问题的方程；
    - （4）原始变分问题的解确实在混合变分问题的空间中；
  - 证明方法：
    - （1）查找已有的结论？
    - （2）很难，只能假设有唯一解
    - （3）直接从原始问题做分部积分，使用新的变量，推出等价关系
    - （4）需要验证边值条件主要是！⚠️
- 马睿师姐讲解 **非线性问题混合变分问题的infsup条件**
  - infsup条件的证明和非线性没有啥关系❓
  - 连续问题的infsup + 插值算子 = 离散的infsup
  - 连续的infsup两步走策略： divDiv，先处理一个，再处理一个
- :pushpin: 关于intro部分的补充
  - 补充差分法等其他CH方程的解法，考虑审稿人的背景；
  - DG和LDG方法并不完全一致，引用文献4个要调研一下；
  - DG，虚拟元，然姐WG等方法要放在一起进行叙述；
  - 补充曲面上的CH方程方法；
  - 可以说一下四阶问题相关的（参考瑞姝师姐给的模型）；
- :pushpin: 列举本文的主要工作，关于引用的文献，需要用siam网站中的参考文献
