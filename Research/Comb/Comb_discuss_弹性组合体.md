...
- 瑞姝师姐继续做关于div和divdiv迹应该如何表达出的工作；
- 马睿师姐：
  - 取$L^2$空间作为对偶空间；
- 关于两个板拼接问题的两种计算方法讨论：
  - 一种在局部坐标系下设置变量，引入界面处的约束条件；
  - 一种是在全局坐标下设置变量，将局部坐标系的刚度矩阵进行转换s
...
- 原始问题基于位移法的变分问题的提法：
  $$
  D(\phi,\psi) + kb(\phi,\psi) = F(\psi)  \quad \forall \psi
  $$
  其中$b(\phi,\psi) = \int_{\Gamma} (u_{3,n} + \tilde{u}_{3,n}) (v_{3,n} + \tilde{v}_{3,n})ds$

- 马睿师姐提出的混合元变分问题的提法：
  $$
  \begin{aligned}
  (\Phi,\Psi)_{C^{-1}} + k\int_{\Gamma}M_{nn}\kappa_{nn}ds + (B^{*}\Psi,\phi) &= 0 \quad & \forall \Psi \\
  (B^{*}\Phi,\psi) &= -(F,\psi) \quad & \forall \psi 
  \end{aligned}
  $$
  注意：

  - 这种提法可以看成是能量的对偶形式，由于考虑弹性连接和固支连接的时候，对应的$M$所在的空间不变，因此考虑只是变分的提法发生改变，将弹性能加到变形形式中；

  - 假设解答充分光滑，那么通过任意取测试函数，是可以使得解答满足原始问题的，这说明解充分光滑的时候解答是等价的，比如可以取在两个板上为0，但是在交界面的转角不为0的函数代入第一行，然后使用分部积分的方法可以得到如下的关系：
    $$
    k\int_{\Gamma}M_{nn}\kappa_{nn}ds + \int_{\Gamma} \kappa_{nn}(u_{3,n}+\tilde{u}_{3,n})ds 
    $$
    在上述关系中需要用到$M_{nn} = \tilde{M}_{nn}$，因此可以得到真实解$u$满足弹性连接的条件。

  - ❓上述问题可以看成是约束极小问题：
    $$
    \operatorname{min}_{Equilibrum} \int_{\Omega} \sigma \varepsilon \mathrm{dx} + kb(\phi,\psi)
    $$
    这和原来固支连接的差别在于多了弹簧连接部分的能量。

  - :pushpin: **缺点：** 关于解的正则性比较弱的时候的等价性比较难证明，因为不能做分部积分！

  - 需要添加条件$M_{nn}$在界面上的积分是有意义的，上述变分形式才是有意义的；

- 瑞姝师姐提出的混合元变分问题的提法:

  - $$
    \begin{aligned}
    (\Phi,\Psi)_{C^{-1}} + (B^{*}\Psi,\phi) &= 0 \quad & \forall \Psi \in \Sigma_1 \\ 
    (B^{*}\Phi,\psi) &= -(F,\psi) \quad & \forall \psi \in V_1
    \end{aligned}
    $$

  - ❓ 注意上述算子$B$和其伴随算子的定义不再和固支连接的含义是一样的，而是使得
    $$
    (B^{*}\Phi,\psi) = D(\phi,\psi) + kb(\phi,\psi)
    $$
    成立，也就是看成是能量的对偶。

- 修改文章的摘要部分介绍的逻辑：

  - 大背景Multistructure, 小背景 plate-plate
  - 经典位移法的结论，从数学Feng，Ciarlet，工程的角度给出
  - 提出混合元的方法，意义和难点，一个是空间的构造，迹比较弱
  - 介绍经典位移法的结论，引出混合元方法
  - 介绍本文的结构
...
- 混合元空间的构造，infsup条件借助fortrin算子和相关假设，正定性条件；
  - fortin算子的投影有界性的另一种表述；
  - 正定性条件只需要在空间的kernal上是成立的；
- 关于条件的名称：
  - **连续性条件**： 指的是在每个单元上满足一定条件后，应该满足什么样的条件，才能形成整个区域上的空间。比如三角形单元上要想全局$H^{1}$,应该满足的连续性条件是在边上是连续的；
  - **边界条件**：边界处应该满足的条件；
  - **界面条件**：特指在两个不同区域的交界面上应该满足的条件；
- 关于结论和展望
  - 本文的工作可以直接推广到弹性连接，借助能量的约束极小，给出变分方程
  - 本文的有限元方法实现将在之后的文章给出，以及与经典问题的比较
  - 本文的结果可以推广到非齐次边值条件，以及三种混合边值条件同时推广
  - 可以推广到其他的弹性组合体问题，板和杆的连接等
